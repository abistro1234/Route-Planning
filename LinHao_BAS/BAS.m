function [path,iteration,map] = BAS(map,start,goal,step,maxFailedAttempts,display,isdynamic,isdynamicdisplay)
isdynamic = false;
...
xbest = start;
x=start;
fbest=fun(xbest,goal,nodes,map,field);
fbest_store=fbest;
pathFound = false;
baknum=0;
...
%         x = reasonableRange(x,ncols,nrows)
        ...
        fleft=fun(xleft,goal,nodes,map,field);
        fright=fun(xright,goal,nodes,map,field);
...
%         x = reasonableRange(x,ncols,nrows)
    ...
    %     fleft=fun(xleft,goal,nodes,map);
    %     fright=fun(xright,goal,nodes,map);
    % %     x = x+step*dir*sign(fleft-fright);
    %     x = double(int32(path(end,:) - step * [sin(dir)  cos(dir)]*sign(fleft-fright)));
    f=fun(x,goal,nodes,map,field);
    nodes = [nodes;x];
    %%%%%%%%%%%
